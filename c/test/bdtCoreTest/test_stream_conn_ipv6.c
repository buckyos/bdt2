	...
	void* userData)
{
	...
	for (size_t ix = 0; ix < recv; ++ix)
	{
		...
		{
			BLOG_DEBUG("got error data on index: %zu, %u expected, but %u got\n", ix + ud->totalRecv, (uint8_t)(ix + ud->totalRecv), buffer[ix]);
		}
	}
	ud->totalRecv += recv;
...
static void OnAcceptConnection(uint32_t result,
...
void testStreamConnIpv6()
{
	const char* localEp[] = { "v6tcp[::1]:10000", NULL };
	const BdtPeerInfo* localPeer = testCreatePeerInfo("local", localEp);
	const char* remoteEp[] = { "v6tcp[::1]:10001", NULL };
	const BdtPeerInfo* remotePeer = testCreatePeerInfo("remote", remoteEp);
	...
	BdtStack* lstack = testCreateBdtStack(userData, localPeer, remotePeer, OnStackEvent);
	BdtStack* rstack = testCreateBdtStack(userData, remotePeer, localPeer, OnStackEvent);
...

	{
		...
		BdtAcceptConnection(rstack, 0, OnAcceptConnection, &userData);
	}
	...
	BFX_BUFFER_HANDLE buf2 = BfxCreateBuffer(10);
	size_t bufSize2 = 0;
	g_recvUserData2.connection = connection2;
	g_recvUserData2.recvBuffer = buf2;
	recvUserData.userData = &g_recvUserData2;
	BdtConnectionRecv(connection2, BfxBufferGetData(buf2, &bufSize2), bufSize2, OnRecv, &recvUserData);
